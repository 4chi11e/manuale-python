...
screen = pygame.display.set_mode(window_size,0,32) # questa è la finestra intera
display = pygame.Surface((900, 600)) # questo è lo spazio in cui voglio disegnare il gioco (eventualmente per lasciare fuori menu, punteggi...)
...
class Mario(pygame.sprite.Sprite):
    def __init__(self, piattaforma, pos, vel_orizz = 5, forza_jump = 18) -> None:
    ...
    def move(self):
        ...
        hit_list = collision_test(self.rect, piattaforma.tile_sprites)
        ...
        for y, row in enumerate(self.game_map):
            for x, tile in enumerate(row):
                if tile != 0:
                    ...
                    self.tile_sprites.add(Tile(image, rect))
        self.tile_sprites.draw(self.image)
...
mario = Mario(piattaforma, [50,50])

all_sprites_list = pygame.sprite.Group()
all_sprites_list.add(piattaforma)
